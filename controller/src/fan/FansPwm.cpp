...
#endif
#if defined(FAN5)
    success = (!configureFanPwm(FAN5_INDEX, FAN5_PWM_CHANNEL, FAN5_PWM_RESOLUTION_BITS, FAN5_PWM_FREQUENCY_HZ, FAN5_PWM_DEFAULT_DUTY,
                                FAN5_PWM_ERROR_DUTY, FAN5_PWM_MIN_DUTY, FAN5_PWM_MAX_DUTY, FAN5_PWM_GPIO_NUM)) ?
              false :
              success;
#endif
#if defined(FAN6)
    success = (!configureFanPwm(FAN6_INDEX, FAN6_PWM_CHANNEL, FAN6_PWM_RESOLUTION_BITS, FAN6_PWM_FREQUENCY_HZ, FAN6_PWM_DEFAULT_DUTY,
                                FAN6_PWM_ERROR_DUTY, FAN6_PWM_MIN_DUTY, FAN6_PWM_MAX_DUTY, FAN6_PWM_GPIO_NUM)) ?
              false :
              success;
#endif
...
bool FansPwm::configureFanPwm(uint8_t index,
                              uint8_t pwmChannel,
                              ...
                              uint8_t gpioNum)
{
    fans[index].minDuty = minDuty;
    fans[index].maxDuty = maxDuty;
    fans[index].defaultDuty = defaultDuty;
    fans[index].errorDuty = errorDuty;
    fans[index].pwmChannel = pwmChannel;

    uint32_t configuredFrequency{ ledcSetup(pwmChannel, pwmFrequency, resolutionBits) };
    ledcAttachPin(gpioNum, pwmChannel);
    setPwm(index, defaultDuty);
